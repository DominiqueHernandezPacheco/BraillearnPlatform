...
const CourseMenu = ({ modules, onSelect, highContrast }) => {
    ...
    useEffect(() => {
        const descriptions = modules
            .map((m, i) => `Módulo ${i + 1}: ${m.title}. ${m.description}`)
            .join('. ');
        const timer = setTimeout(() => {
            speak(
                `Menú de cursos. Hay ${modules.length} módulos disponibles. ` +
                `${descriptions}. ` +
                `Usa Tab para navegar y Enter para abrir un módulo.`,
                true
            );
        }, 400);
    ...
    const cardHoverClass = highContrast
    ...
    return (
        <section
            id="cursos-top"
            className={`min-h-screen ${bgClass} px-6 pt-24 pb-12 animate-fadeIn transition-colors duration-300`}
        >
            <div className="container mx-auto max-w-6xl">
                <h2 className={`text-4xl font-bold text-center mb-4 ${textClass}`}>
                    Camino del Braille
                </h2>
                <p className={`text-center mb-12 text-lg ${textSubClass}`}>
                    Selecciona un módulo para comenzar tu aventura.
                </p>
